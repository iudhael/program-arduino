/*#include <LiquidCrystal_I2C.h>
  ...
  void loop() {
  const char spinner[] = {'\\', '|', '/'};  // Animation de chargement
  const int spinnerLength = sizeof(spinner) / sizeof(spinner[0]);
  unsigned long currentTime = millis();

  lcd.setCursor(10, 2);         // Positionner le curseur au début de la 1ère ligne
...
/*
   Arduino LCD I2C Tutorial by 'Ogbugbu Technologies"
   Please visit my YouTube channel from this link for more Tutorials
   https://www.youtube.com/@OkahIraRossana
*/
...
void loop()
{
  ...
  sprintf(buffer, "CHARGEMENT:%3d%% ", cpu_gauge);    // set a string as CPU: XX%, with the number always taking at least 3 character
  lcd.print(buffer);                          // print the string on the display
  lcd.write(byte(0));                         // print warning character

  lcd.setCursor(2, 2);             // move the cursor to the next line
